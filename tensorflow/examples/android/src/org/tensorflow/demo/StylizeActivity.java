...
public class StylizeActivity extends CameraActivity implements OnImageAvailableListener {
  ...
  // private static final String MODEL_FILE = "file:///android_asset/stylize_quantized_v1.pb";
  // private static final String INPUT_NODE = "input";
  // private static final String STYLE_NODE = "style_num";
  // private static final String OUTPUT_NODE = "transformer/expand/conv3/conv/Sigmoid";

  private static final String MODEL_FILE = "file:///android_asset/export.pb";
  private static final String INPUT_NODE = "input";
  private static final String INPUT_SHAPE_NODE = "input_shape";
  ...
  private static final int[] SIZES = {1, 2, 4, 8, 16, 32, 64, 128, 192, 256, 512, 1024};
  ...
  private int[] intValuesOut;
  private float[] floatValues;
  private float[] floatValuesOut;
          ...
          new OnClickListener() {
            ...
            public void onClick(final View v) {
              if (croppedBitmap != null) {
                ...
                ImageUtils.saveBitmap(croppedBitmap, "DL_input" + frameNum + ".png");
                Toast.makeText(
                        StylizeActivity.this,
                        "Saved image to: /sdcard/tensorflow/" + "DL_input" + frameNum + ".png",
                        Toast.LENGTH_LONG)
                    .show();
              }              
              if (textureCopyBitmap != null) {
                // TODO(andrewharp): Save as jpeg with guaranteed unique filename.
                ImageUtils.saveBitmap(textureCopyBitmap, "DL_run" + frameNum + ".png");
                Toast.makeText(
                        StylizeActivity.this,
                        "Saved image to: /sdcard/tensorflow/" + "DL_run" + frameNum + ".png",
                        Toast.LENGTH_LONG)
                    .show();
              }
            }
  ...
  public void onImageAvailable(final ImageReader reader) {
    ...
    try {
      ...
      if (desiredSize != initializedSize) {
        ...
        outBitmap = Bitmap.createBitmap(desiredSize * 4, desiredSize * 4, Config.ARGB_8888);
        ...
        intValuesOut = new int[desiredSize * desiredSize * 16];
        floatValues = new float[desiredSize * desiredSize * 3];
        floatValuesOut = new float[desiredSize * desiredSize * 3 * 16];
    ...
    runInBackground(
        new Runnable() {
          ...
          public void run() {
            ...
            stylizeImage(croppedBitmap, outBitmap);
            lastProcessingTimeMs = SystemClock.uptimeMillis() - startTime;

            textureCopyBitmap = Bitmap.createBitmap(outBitmap);
  ...
  private void stylizeImage(final Bitmap bitmap, final Bitmap outbitmap) {
    ++frameNum;
    ...
    input_shape[0] = bitmap.getHeight();
    input_shape[1] = bitmap.getWidth();
    input_shape[2] = 3;

    // Copy the input data into TensorFlow.
    inferenceInterface.fillNodeFloat(
        INPUT_NODE, new int[] {1, bitmap.getWidth(), bitmap.getHeight(), 3}, floatValues);
    inferenceInterface.fillNodeInt(INPUT_SHAPE_NODE, new int[] {3}, input_shape);
    inferenceInterface.runInference(new String[] {OUTPUT_NODE});
    inferenceInterface.readNodeFloat(OUTPUT_NODE, floatValuesOut);

    for (int i = 0; i < intValuesOut.length; ++i) {
      intValuesOut[i] =
          0xFF000000
              | (((int) (floatValuesOut[i * 3] * 255)) << 16)
              | (((int) (floatValuesOut[i * 3 + 1] * 255)) << 8)
              | ((int) (floatValuesOut[i * 3 + 2] * 255));
    }

    outbitmap.setPixels(intValuesOut, 0, outbitmap.getWidth(), 0, 0, outbitmap.getWidth(), outbitmap.getHeight());
  }
